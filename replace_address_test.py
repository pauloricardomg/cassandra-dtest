...
class TestReplaceAddress(Tester):
    ...
    def _setup(self, n=3, opts=None, enable_byteman=False, mixed_versions=False):
        ...
        n = len(self.cluster.nodelist())  # n may be a list of nodes per dc
        self.cluster.set_batch_commitlog(enabled=True)
        self.query_node = self.cluster.nodelist()[0]
        self.replaced_node = self.cluster.nodelist()[n - 1]
    ...
    def _test_restart_failed_replace(self, mode):
        ...
        self.query_node.byteman_submit(['./stream_failure.btm'])
    ...
    def replace_with_insufficient_replicas_test(self):
        ...
        """
        self.ignore_log_patterns.extend([r'Unable to find sufficient sources for streaming range'])
        self._setup(n=3)
