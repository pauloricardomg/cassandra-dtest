...
class TestStreaming(Tester):
    ...
    def extract_invalidated_keys_and_ranges(self, node):
        invalidated_logs = node.grep_log('row cache entries from')
        invalidated_keys = 0
        invalidated_ranges = 0
        for log in invalidated_logs:
            count = re.search(r'Invalidated (\d+) row cache entries', str(log), re.IGNORECASE)
            if count:
                invalidated_keys += int(count.group(1))
            count = re.search(r'from (\d+) ranges', str(log), re.IGNORECASE)
            if count:
    ...
    @since('2.1')
    def test_stream_row_cache_invalidation(self):
